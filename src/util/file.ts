...
export const loadFile = <T = object>(path: string, system = true): T | false | undefined => {
...
export const existsFile = (path: string, system = true) => {
	const rePath = system ? `${os.homedir()}/${path}` : path
	loggerInfo(rePath)
 ...
 * @return {*}
 */
export const writeFile = (path: string, fileName: string, file: object, system = true) => {
	const rePath = system ? `${os.homedir()}/${path}` : path
	loggerInfo(rePath)
	try {
